...
interface SuspectsBoardProps {
  round: RoundCode;
  /** Absent in Round 01: the engine only publishes the roster with Round 02. */
  vote: VoteSnapshot | undefined;
}

/** Deterministic monogram from the published name. No new facts involved. */
...
function DossierRow({
  ...
  return (
    <li
      className={cn(
        "flex items-start gap-3 border-b border-line/50 px-3.5 py-4 last:border-b-0 transition-colors sm:px-5",
 ...
 * than rendering an empty board that implies the investigation has subjects.
 */
